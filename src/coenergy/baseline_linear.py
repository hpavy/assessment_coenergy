...
Decomposes each time series into 10 equal parts, computes mean/std/max/min
for each part, concatenates into a feature vector, and trains a linear model.
"""
...
def extract_features(x: torch.Tensor, n_parts: int = 10) -> np.ndarray:
    ...
    features = torch.stack([means, stds, maxs, mins], dim=-1)  # [N, C, n_parts, 4]
    features = features.reshape(N, -1)  # [N, C * n_parts * 4]
